...
- Routine manager inventory requests shared status with `probe: false`: it reads manifest, durable activation and existing runtime health without starting dependency subprocesses. Dependency checks remain part of activation and the model's explicit `capability_status` tool; inventory must not imply a fresh dependency check.
...
- Final lock review identified a remaining check-then-unlink race in the initial custom lock implementation. That implementation was removed in favor of existing Core Flock. The focused live-lease/concurrent-process regressions failed before the replacement. Afterward, `bun test test/capability/manager-store.test.ts` — **9 pass, 0 fail, 35 assertions**. Eight real subprocesses contend for an expired lease: one save succeeds, seven reject stale revisions, and only one server is persisted. A live shared lease prevents writes until released.
- Final shared-lock verification: Core `bun test test/util/flock.test.ts` — **10 pass, 0 fail, 26 assertions**, including process contention and crashed-owner recovery. Host `bun typecheck` passed again after the lock replacement.
- Native integration exposed slow inventory caused by sequential dependency probes for every installed pack. A new regression first observed 12 unwanted probe commands, then passed with zero inventory probes while retaining failed runtime remediation. After the fast-list change, the full Core capability command above passed **84 tests, 312 assertions**; the real HTTP manager flow passed **1 test, 23 assertions** (2.22 seconds test time). Activation and model status probing are unchanged.
...
- Attachment is atomic per file, not a cross-file crash transaction. Staging order preserves safe always-on behavior if a crash occurs between the pack and exposure writes. Shared Flock uses a 60-second heartbeat expiry; a crashed owner's lease can therefore temporarily require retrying after expiry. Manager lock acquisition is bounded to five seconds per attempt. Obsolete experimental `.manager-lock` files are no longer read or required and are not automatically deleted.
